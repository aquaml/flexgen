...
class TorchTensor:
    ...
    def __init__(self, shape, dtype, data, device, name=None, is_aqua_tensor=False):
    ...
    @classmethod
    def create_from_torch(cls, data, device, name=None, is_aqua_tensor=False):
        return cls(data.shape, data.dtype, data, device, name=name, is_aqua_tensor=is_aqua_tensor)
...
class TorchDevice:
    ...
    def __init__(self, name, mem_capacity=None, flops=None, use_aqua=False):
        self.name = name
...
class TorchMixedDevice:
    ...
    def init_cache_one_gpu_batch(self, config, task, policy):
        ...
        if policy.cache_disk_percent == 0:
            ...
            len_cpu = 0
            len_cache_gpu = shape[SEG_DIM]  - len_gpu
            len_disk = 0
        else:
            len_gpu = int(shape[SEG_DIM] * policy.cache_gpu_percent / 100) // num_head * num_head
            len_cache_gpu = int(shape[SEG_DIM] * policy.cache_cpu_percent / 100) // num_head * num_head
            len_disk = shape[SEG_DIM] - len_gpu - len_cpu
            len_cpu = 0
        lens = [len_gpu, len_cpu, len_disk, len_cache_gpu]
