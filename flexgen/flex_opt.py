...
class OptLM:
    ...
    def generate(self,
        ...
        if not self.allocated_cache:
            for j in range(num_layers):
                for k in range(num_gpu_batches):
                    self.init_cache(j, k)
            self.allocated_cache = True
...
def log_outputs(costs, num_prompts, prompt_len, gen_len, cut_gen_len, tokenizer, gpu, cpu, output_ids, opt_config, cache_size, hidden_size):
    ...
    if args.verbose >= 1:
        print('LOG TIME: {}'.format(int(time.time())))
        print(log_str)
...
def run_flexgen(args):
    ...
    cache_gpu = TorchDevice("cuda:0")
    cache_gpu.use_aqua = True

    print("Setting up cache space to use aqua with dynamic policy")
    dynamic_policy = aqua.dynamic_policy.dynamic_policy('localhost', 8080)
    aqua.responsive_manager.set_policy(dynamic_policy)
    
    cpu = TorchDevice("cpu")
    disk = TorchDisk(args.offload_dir)
    env = ExecutionEnv(gpu=gpu, cpu=cpu, disk=disk, mixed=TorchMixedDevice([gpu, cpu, disk, cache_gpu]), cache_gpu=cache_gpu)
    ...
    print("Overlapping toggle: {}".format(policy.overlap))
    print("init weight...")
    ...
    try:
        ...
        while int(time.time()) - start_time < 8 * 60:
            ...
            # Log output
            log_outputs(costs, num_prompts, prompt_len, gen_len, cut_gen_len, tokenizer, gpu, cpu, output_ids, opt_config, cache_size, hidden_size)
        model.delete_cleanup()
    finally:
...
if __name__ == "__main__":
    print("This is linked to pip path")
